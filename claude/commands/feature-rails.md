...
### Step 2: Ask Clarifying Questions (Iteratively Until Complete)
Use **AskUserQuestion** tool to gather details. Ask **2-4 questions at a time** - don't overwhelm. **Continue asking rounds of questions until every aspect of the feature is fully specified with no ambiguity remaining.** There is no fixed number of rounds — keep going until done.

Start with high-level questions and progressively drill into specifics. Use the categories below as a guide, but adapt based on the feature and the user's answers. Skip categories that don't apply, and add new questions as gaps emerge.

**Suggested question categories (adapt as needed):**

- **High-Level Scope:** Core problem, target users, must-haves vs nice-to-haves
- **Technical Approach:** Models/tables, integration with existing architecture, external APIs, data flow
- **User Experience:** Where it lives in the app, user workflow (step by step), UI components, user feedback
- **Data & Logic:** Specific data to store, validations, business rules, authorization/permissions, edge cases
- **Testing & Polish:** Critical paths, error scenarios, performance considerations

### Step 3: Iterate Until All Questions Are Resolved
- **After each round of answers, evaluate whether any ambiguity remains.** If it does, ask another round of questions. Do NOT move to the summary step until you are confident every aspect is fully specified.
...
- **If a user's answer raises new questions or reveals unstated assumptions, ask about those too**
...
- **Confirm there are NO open questions remaining.** If any remain, go back to Step 3 and ask more questions before proceeding.
...
- **NEVER create an issue with an "Open Questions" section — all questions must be resolved before issue creation**
- Stop asking questions after a fixed number of rounds if ambiguity remains
